...
static void prepareSyncPacket(void);
static void prepareInitFeedbackPacket(uint8_t numberOfSlot);
static uint8_t findFreeTimeSlot(void);
...
static inline void initPPI()
{
	...
	nrf_ppi_channel_endpoint_setup(NRF_PPI_CHANNEL0, (uint32_t) &rtc0->RTCx->EVENTS_COMPARE[0], (uint32_t) &rtc1->RTCx->TASKS_START);
	nrf_ppi_channel_enable(NRF_PPI_CHANNEL0);

	// when SUF is over start again
	nrf_ppi_channel_endpoint_setup(NRF_PPI_CHANNEL1, (uint32_t) &rtc0->RTCx->EVENTS_COMPARE[1], (uint32_t) &rtc0->RTCx->TASKS_CLEAR);
	nrf_ppi_channel_enable(NRF_PPI_CHANNEL1);

	// when SUF is over then STOP and CLEAR RTC1
	nrf_ppi_channel_and_fork_endpoint_setup(NRF_PPI_CHANNEL2, (uint32_t) &rtc0->RTCx->EVENTS_COMPARE[1], (uint32_t) &rtc1->RTCx->TASKS_STOP, (uint32_t) &rtc1->RTCx->TASKS_CLEAR);
	...
	nrf_ppi_channel_endpoint_setup(NRF_PPI_CHANNEL3, (uint32_t) &rtc1->RTCx->EVENTS_COMPARE[0], (uint32_t) &rtc1->RTCx->TASKS_CLEAR);
	...
	if(radio->isRxIdleState())
	{
		...
		if(radio->checkCRC())
		{
			...
			else if(isPacketInit(packetAsInit))
			{
				...
				addSensor(freeSlot);
				prepareInitFeedbackPacket(freeSlot);
